...
export default function ForgotPasswordScreen() {
  const { sendPasswordReset } = useAuth(); const palette = useAppTheme();
  const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { email: '' } });
  const submit = handleSubmit(async ({ email }) => { try { await sendPasswordReset(email.trim()); Alert.alert('Email sent', 'Use the link in your inbox to reset your password.'); } catch (error) { Alert.alert('Could not send email', getErrorMessage(error)); } });
