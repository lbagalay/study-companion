...
const schema = z.object({
  ...
  preferredName: z
    .string()
    .trim()
    .min(1, 'Enter the name you want to see in the app.')
...
export default function RegisterScreen() {
  ...
  const {
    ...
    defaultValues: { fullName: '', preferredName: '', email: '', password: '' },
  });
  const submit = handleSubmit(async (values) => {
    try {
      const signedIn = await signUp({ ...values, email: values.email.trim() });
      if (!signedIn) {
        Alert.alert('Check your inbox', 'Confirm your email, then return to sign in.');
  ...
  return (
    <AuthScreen
      description="A calm space for classes, deadlines, and study plans."
