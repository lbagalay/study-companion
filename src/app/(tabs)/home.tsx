...
export default function HomeScreen() {
  const palette = useAppTheme(); const router = useRouter(); const { width } = useWindowDimensions(); const compact = width < 640; const { user } = useAuth(); const profile = useProfile(user?.id); const subjects = useSubjects(); const schedules = useSchedules(); const assignments = useAssignments(); const exams = useExams(); const sessions = useSessions();
  ...
  return <View style={[styles.screen, { backgroundColor: palette.background }]}><EditorialBackdrop /><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.quickActions}>
      <QuickAction color={palette.accent} icon="checkbox-outline" index="01" label="Activity" onPress={() => router.push('/assignments/create')} surface={palette.accentSoft} textColor={palette.text} />
      <QuickAction color={palette.lavender} icon="school-outline" index="02" label="Exam or quiz" onPress={() => router.push('/exams/create')} surface={palette.lavenderSoft} textColor={palette.text} />
      <QuickAction color={palette.warning} icon="book-outline" index="03" label="Study plan" onPress={() => router.push('/sessions/create-plan')} surface={palette.peachSoft} textColor={palette.text} />
    </View>
    <Section color={palette.text} index="01" tint={palette.lavenderSoft} title="Today">{todayClasses.map((item) => { const subject = bySubject.get(item.subject_id); const current = isClassCurrent(item.start_time, item.end_time, now); return <EntityCard accent={subject?.color} badge={current ? 'NOW' : clock(item.start_time)} key={item.id} metadata={item.room || subject?.room} onPress={() => router.push({ pathname: '/schedule/[id]', params: { id: item.id } })} subtitle={`${clock(item.start_time)}–${clock(item.end_time)}`} title={subject?.name ?? 'Class'} />; })}{!todayClasses.length ? <FeedbackState message="No classes are scheduled today." title="A clear day" /> : null}</Section>
...
const styles = StyleSheet.create({
  ...
  hero: { borderRadius: radii.xl, borderWidth: 1, boxShadow: '0 18px 42px rgba(14, 27, 72, 0.10)', overflow: 'hidden', padding: spacing.xl },
