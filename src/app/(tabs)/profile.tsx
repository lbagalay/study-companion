...
import { cancelAllNotifications, getNotificationStatus, requestNotificationPermission, type NotificationStatus } from '@/lib/notifications';
...
function ProfileEditor({ profile, userId }: { profile: Profile; userId: string }) {
  ...
  const refreshNotificationStatus = () => void getNotificationStatus().then(setNotificationStatus).catch(() => setNotificationStatus(null));
  ...
  const save = useMutation({ mutationFn: async (values: Values) => { if (!values.notifications) await cancelAllNotifications(); return updateProfile(userId, { full_name: values.fullName.trim(), preferred_name: values.preferredName.trim(), notifications_enabled: values.notifications }); }, onSuccess: async () => { await client.invalidateQueries({ queryKey: ['profile', userId] }); refreshNotificationStatus(); Alert.alert('Saved', 'Your preferences are up to date.'); }, onError: (e) => Alert.alert('Could not save profile', getErrorMessage(e)) });
  const enableReminders = useMutation({ mutationFn: async () => { const granted = await requestNotificationPermission(); if (!granted) throw new Error('Notification permission was not granted.'); await updateProfile(userId, { notifications_enabled: true }); }, onSuccess: async () => { await client.invalidateQueries({ queryKey: ['profile', userId] }); refreshNotificationStatus(); Alert.alert('Reminders enabled', 'This device is ready for activity, exam, quiz, and study reminders.'); }, onError: (e) => { refreshNotificationStatus(); Alert.alert('Could not enable reminders', getErrorMessage(e)); } });
  return <ScreenContainer><ScreenHeader description={user?.email} title="Profile" /><View style={styles.form}>
    <View style={[styles.notificationCard, { backgroundColor: palette.surfaceAlt, borderColor: palette.border }]}>
      <View style={[styles.notificationIcon, { backgroundColor: palette.accentSoft }]}><Ionicons color={palette.accent} name={notificationStatus?.subscribed ? 'notifications' : 'notifications-outline'} size={24} /></View>
      <View style={styles.notificationCopy}><Text style={[styles.notificationTitle, { color: palette.text }]}>{notificationStatus?.subscribed ? 'Reminders are ready' : 'Never miss what’s next'}</Text><Text style={[styles.notificationDetail, { color: palette.textMuted }]}>{notificationStatus?.detail ?? 'Checking notification support on this device…'}</Text></View>
      {notificationStatus?.supported && !notificationStatus.subscribed && !notificationStatus.requiresInstall && notificationStatus.permission !== 'denied' ? <AppButton label="Enable this device" loading={enableReminders.isPending} onPress={() => enableReminders.mutate()} style={styles.notificationButton} variant="secondary" /> : null}
    </View>
...
const styles = StyleSheet.create({
  ...
  notificationCard: { borderRadius: radii.xl, borderWidth: 1, gap: spacing.md, padding: spacing.lg },
  notificationIcon: { alignItems: 'center', borderRadius: radii.pill, height: 46, justifyContent: 'center', width: 46 },
