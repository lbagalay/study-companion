...
export default function ScheduleScreen() {
  ...
  return <EntityList addLabel="Add class manually" description="Your recurring weekly class timetable." empty={!schedules.data?.length} emptyMessage="Add a class manually or upload your study load so today’s schedule appears on Home." error={schedules.error} loading={schedules.isLoading} onAdd={() => router.push('/schedule/create')} onRefresh={() => void Promise.all([schedules.refetch(), subjects.refetch()])} onSecondaryAdd={() => router.push('/subjects/import')} refreshing={schedules.isRefetching} secondaryAddLabel="Upload study load" title="Schedule">
