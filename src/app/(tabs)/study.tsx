...
export default function StudyScreen() {
  ...
  return <EntityList addLabel="Add material" description="Everything you need for focused study sessions." empty={false} emptyMessage="" error={materials.error ?? notes.error ?? sessions.error} loading={loading} onAdd={() => router.push('/materials/create')} onRefresh={() => void Promise.all([materials.refetch(), notes.refetch(), sessions.refetch(), subjects.refetch()])} refreshing={materials.isRefetching || notes.isRefetching || sessions.isRefetching} title="Study">
    <View style={[styles.ritual, { backgroundColor: palette.lavenderSoft, borderColor: palette.border }]}>
      ...
      <View style={styles.actions}><AppButton icon="document-text-outline" label="New note" onPress={() => router.push('/notes/create')} style={styles.action} variant="secondary" /><AppButton icon="sparkles-outline" label="Plan study" onPress={() => router.push('/sessions/create-plan')} style={styles.action} variant="secondary" /></View>
    </View>
    ...
    {!sessions.data?.length ? <FeedbackState message="Create a study plan from an upcoming exam." title="No sessions planned" /> : null}<AppButton icon="arrow-forward-circle-outline" label="View all sessions" onPress={() => router.push('/sessions')} variant="ghost" />
