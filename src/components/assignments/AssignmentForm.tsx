...
const reminderChoices = [{ label: '1 week before', value: 10080 }, { label: '3 days before', value: 4320 }, { label: '1 day before', value: 1440 }, { label: '3 hours before', value: 180 }, { label: '1 hour before', value: 60 }] as const;
const schema = z.object({ subject_id: z.string().min(1, 'Choose a subject.'), title: z.string().trim().min(1, 'Enter an assignment title.').max(150, 'Keep the title under 150 characters.'), description: z.string().trim().max(3000, 'Keep the description under 3,000 characters.'), due_at: z.string().datetime('Choose a valid due date.'), priority: z.enum(['LOW', 'MEDIUM', 'HIGH', 'URGENT']), status: z.enum(['NOT_STARTED', 'IN_PROGRESS', 'COMPLETED']), reminder_offsets: z.array(z.number().int().positive()).min(1, 'Choose at least one reminder.'), notes: z.string().trim().max(3000, 'Keep notes under 3,000 characters.') });
...
export function AssignmentForm({ id }: { id?: string }) {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { subject_id: '', title: '', description: '', due_at: addDays(new Date(), 1).toISOString(), priority: 'MEDIUM', status: 'NOT_STARTED', reminder_offsets: [1440, 180, 60], notes: '' } });
  useEffect(() => { if (!id && subjects.data?.[0]) reset((v) => ({ ...v, subject_id: v.subject_id || subjects.data![0].id })); }, [id, reset, subjects.data]);
  useEffect(() => { if (item.data) reset({ subject_id: item.data.subject_id, title: item.data.title, description: item.data.description, due_at: item.data.due_at, priority: item.data.priority, status: item.data.status, reminder_offsets: item.data.reminder_offsets, notes: item.data.notes }); }, [item.data, reset]);
  const save = useMutation({ mutationFn: async (v: Values) => { await cancelNotifications(item.data?.notification_ids ?? []); const saved = await saveAssignment({ ...v, completed_at: v.status === 'COMPLETED' ? item.data?.completed_at ?? new Date().toISOString() : null, notification_ids: [] }, id); if (v.status === 'COMPLETED') return saved; const notificationIds = await scheduleReminders(v.title, 'An activity deadline is approaching.', v.due_at, v.reminder_offsets); return updateAssignment(saved!.id, { notification_ids: notificationIds }); }, onSuccess: async () => { await client.invalidateQueries({ queryKey: keys.assignments }); router.back(); }, onError: (e) => Alert.alert('Could not save assignment', getErrorMessage(e)) });
  ...
  return <ScreenContainer><ScreenHeader back description="Set a clear due date and priority." title={id ? 'Edit assignment' : 'New assignment'} /><View style={styles.form}>
    ...
    <Controller control={control} name="due_at" render={({ field }) => <DateTimeField error={errors.due_at?.message} label="Due date and time" onChange={field.onChange} value={field.value} />} />
    ...
    <AppButton icon={id ? 'checkmark-circle-outline' : 'add-circle-outline'} label={id ? 'Save changes' : 'Add assignment'} loading={save.isPending} onPress={handleSubmit((v) => save.mutate(v))} />
    {id ? <AppButton icon="trash-outline" label="Delete assignment" loading={remove.isPending} onPress={() => Alert.alert('Delete assignment?', 'This cannot be undone.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: () => remove.mutate() }])} variant="danger" /> : null}
