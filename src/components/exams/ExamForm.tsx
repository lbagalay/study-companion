...
export function ExamForm({ id }: { id?: string }) {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { subject_id: '', title: '', type: 'EXAM', exam_at: addDays(new Date(), 7).toISOString(), room: '', coverage: '', notes: '', status: 'UPCOMING' } });
  useEffect(() => { if (!id && subjects.data?.[0]) reset((v) => ({ ...v, subject_id: v.subject_id || subjects.data![0].id })); }, [id, reset, subjects.data]);
  useEffect(() => { if (item.data) reset({ subject_id: item.data.subject_id, title: item.data.title, type: item.data.type, exam_at: item.data.exam_at, room: item.data.room, coverage: item.data.coverage, notes: item.data.notes, status: item.data.status }); }, [item.data, reset]);
  const save = useMutation({ mutationFn: async (v: Values) => { await cancelNotifications(item.data?.notification_ids ?? []); const saved = await saveExam({ ...v, notification_ids: [] }, id); if (v.status !== 'UPCOMING') return saved; const notificationIds = await scheduleExamReminders(v.title, v.exam_at, item.data?.reminder_offsets ?? [10080, 4320, 1440]); return updateExam(saved!.id, { notification_ids: notificationIds }); }, onSuccess: async () => { await client.invalidateQueries({ queryKey: keys.exams }); router.back(); }, onError: (e) => Alert.alert('Could not save exam', getErrorMessage(e)) });
