...
export function MaterialForm({ id }: { id?: string }) {
  const router = useRouter(); const client = useQueryClient(); const palette = useAppTheme(); const subjects = useSubjects(); const [asset, setAsset] = useState<DocumentPicker.DocumentPickerAsset | null>(null); const [inspectedPdf, setInspectedPdf] = useState<InspectedPdf | null>(null); const [inspecting, setInspecting] = useState(false); const [formError, setFormError] = useState<string | null>(null);
  ...
  useEffect(() => { if (!id && subjects.data?.[0] && !getValues('subject_id')) setValue('subject_id', subjects.data[0].id, { shouldValidate: true }); }, [getValues, id, setValue, subjects.data]);
  ...
  return <ScreenContainer><ScreenHeader back description="Files stay in a private, user-scoped bucket." title={id ? 'Material details' : 'Add material'} /><View style={styles.form}>
    <Controller control={control} name="subject_id" render={({ field }) => <SubjectField error={errors.subject_id?.message} onChange={(value) => { setFormError(null); field.onChange(value); }} value={field.value} />} />
    {!subjects.isLoading && !subjects.data?.length ? <AppButton label="Create a subject first" onPress={() => router.push('/subjects/create')} variant="secondary" /> : null}
    <Controller control={control} name="title" render={({ field }) => <FormField error={errors.title?.message} label="Title" onChangeText={(value) => { setFormError(null); field.onChange(value); }} value={field.value} />} />
    <Controller control={control} name="type" render={({ field }) => <ChoiceField choices={materialTypes} label="Type" onChange={(value) => { setFormError(null); field.onChange(value); setAsset(null); setInspectedPdf(null); }} value={field.value} />} />
    {['LINK', 'VIDEO_LINK'].includes(type) ? <Controller control={control} name="external_url" render={({ field }) => <FormField autoCapitalize="none" error={errors.external_url?.message} keyboardType="url" label="URL" onChangeText={(value) => { setFormError(null); field.onChange(value); }} value={field.value} />} /> : type !== 'NOTE' ? <><AppButton label={inspecting ? 'Checking PDF' : asset ? 'Choose a different file' : type === 'PDF' ? 'Choose PDF' : 'Choose file'} loading={inspecting} onPress={() => void pick()} variant="secondary" />{inspectedPdf ? <Text style={[styles.file, { color: palette.textMuted }]}>{inspectedPdf.fileName} · {inspectedPdf.pageCount} pages · {formatFileSize(inspectedPdf.fileSize)}</Text> : asset ? <Text style={[styles.file, { color: palette.textMuted }]}>{asset.name}</Text> : item.data?.file_url ? <Text style={[styles.file, { color: palette.textMuted }]}>{item.data.file_name ? `${item.data.file_name}${item.data.page_count ? ` · ${item.data.page_count} pages` : ''}` : 'Existing upload will be kept.'}</Text> : null}</> : null}
    ...
    {formError ? <Text accessibilityRole="alert" style={[styles.error, { backgroundColor: palette.accentSoft, borderColor: palette.border, color: palette.danger }]}>{formError}</Text> : null}
    <AppButton disabled={inspecting || (!id && !subjects.data?.length)} label={type === 'PDF' ? id ? 'Save and open PDF' : 'Upload and open PDF' : id ? 'Save changes' : 'Add material'} loading={save.isPending} onPress={handleSubmit((v) => save.mutate(v), (invalid) => setFormError(invalid.subject_id?.message ?? invalid.title?.message ?? invalid.external_url?.message ?? 'Review the highlighted fields and try again.'))} />
...
const styles = StyleSheet.create({ form: { gap: spacing.md, paddingBottom: spacing.xxl }, file: typography.body, error: { ...typography.caption, borderRadius: 12, borderWidth: 1, padding: spacing.md } });
