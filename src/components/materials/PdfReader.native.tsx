...
export function PdfReader({ materialId }: { initialPage?: number; materialId: string }) {
  ...
  if (!material.data?.file_url || material.data.type !== 'PDF')
    return <FeedbackState message="This material is not a stored PDF." title="PDF unavailable" />;
  return (
    <ScreenContainer>
      <ScreenHeader
        back
        description="Page tracking is currently available in the installed PWA."
        title={material.data.title}
      />
      <AppButton
        label="Open in system PDF viewer"
        onPress={() =>
          void (async () => {
            try {
              await Linking.openURL(await getMaterialUrl(material.data!.file_url!, 3600));
            } catch (error) {
              Alert.alert('Could not open PDF', getErrorMessage(error));
            }
          })()
        }
      />
