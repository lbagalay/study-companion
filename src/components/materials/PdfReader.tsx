...
function ContinuousPdfPage({
  ...
  useEffect(() => {
    ...
    if (!context) {
      setRenderError(
        'The browser could not create the PDF canvas.',
      );
...
export function PdfReader({
  ...
  const visibleRatiosRef =
    useRef(
      new Map<number, number>(),
    );

  const lastVisiblePageRef =
    useRef(1);

  const pendingScrollPageRef =
    useRef<number | null>(
      null,
    );

  const suppressVisibilityRef =
    useRef(false);
      ...
      (
        ...
        saveQueue.current =
          saveQueue.current
            ...
            .then(async () => {
              const saved =
                await updatePdfReadingProgress(
                  materialId,
                  page,
                  pageCount,
                );
  ...
  const handlePageVisibility =
    useCallback(
      (
        ...
        visibleRatiosRef.current.forEach(
          (
            visibleRatio,
            page,
          ) => {
  ...
  if (material.isLoading) {
    return (
      <FeedbackState
        loading
        message="Checking your private material."
        title="Opening PDF reader"
      />
    );
  }
  ...
  if (
    !material.data ||
    material.data.type !== 'PDF' ||
    !material.data.file_url
  ) {
    return (
      <FeedbackState
        message="This material is not a stored PDF."
