...
const schema = z.object({ subject_id: z.string().min(1, 'Choose a subject.'), title: z.string().trim().min(1, 'Enter a note title.').max(150, 'Keep the title under 150 characters.'), content: z.string().max(50000, 'Keep the note under 50,000 characters.'), favorite: z.boolean() }); type Values = z.infer<typeof schema>;
export function NoteForm({ id }: { id?: string }) {
  ...
  if (id && item.isLoading) return <FeedbackState loading message="Loading note." title="One moment" />;
  return <ScreenContainer><ScreenHeader back description="Keep lightweight notes close to their subject." title={id ? 'Edit note' : 'New note'} /><View style={styles.form}>
    ...
    <Controller control={control} name="content" render={({ field }) => <FormField label="Note" multiline onChangeText={field.onChange} style={styles.note} value={field.value} />} />
    <Controller control={control} name="favorite" render={({ field }) => <ChoiceField choices={[{ label: 'Standard', value: 'NO' }, { label: 'Favorite', value: 'YES' }]} label="Favorite" onChange={(v) => field.onChange(v === 'YES')} value={field.value ? 'YES' : 'NO'} />} />
    <AppButton label={id ? 'Save changes' : 'Create note'} loading={save.isPending} onPress={handleSubmit((v) => save.mutate(v))} />
    {id ? <AppButton label="Delete note" loading={remove.isPending} onPress={() => Alert.alert('Delete note?', 'This cannot be undone.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: () => remove.mutate() }])} variant="danger" /> : null}
  </View></ScreenContainer>;
}
const styles = StyleSheet.create({ form: { gap: spacing.md, paddingBottom: spacing.xxl }, note: { minHeight: 240 } });
