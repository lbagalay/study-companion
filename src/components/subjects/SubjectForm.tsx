...
export function SubjectForm({ id }: { id?: string }) {
  ...
  if (id && subject.isLoading) return <FeedbackState loading message="Loading subject." title="One moment" />;
  return <ScreenContainer><ScreenHeader back description={id ? 'Update details or review this subject.' : 'Organize classes and study work by subject.'} title={id ? 'Subject details' : 'New subject'} />
    <View style={styles.form}>
      <Controller control={control} name="name" render={({ field }) => <FormField error={errors.name?.message} label="Subject name" onChangeText={field.onChange} value={field.value} />} />
      ...
      <Controller control={control} name="color" render={({ field }) => <ChoiceField choices={colors.map((value) => ({ label: value, value }))} label="Color" onChange={field.onChange} value={field.value} />} />
      <AppButton label={id ? 'Save changes' : 'Add subject'} loading={save.isPending} onPress={handleSubmit((values) => save.mutate(values))} />
      {id ? <AppButton label="Delete subject" loading={remove.isPending} onPress={confirmDelete} variant="danger" /> : null}
    </View>
  </ScreenContainer>;
}
