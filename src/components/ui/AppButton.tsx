...
import { ActivityIndicator, Animated, Platform, Pressable, StyleSheet, Text, type StyleProp, type ViewStyle } from 'react-native';
...
export function AppButton({ disabled, icon, label, loading = false, onPress, onPressIn, onPressOut, style, variant = 'primary', ...props }: Props) {
  ...
  const textColor = variant === 'danger' ? '#FFFFFF' : variant === 'primary' ? palette.text : palette.accentStrong;
...
const styles = StyleSheet.create({
  ...
  button: { alignItems: 'center', borderRadius: radii.md, borderWidth: 1, flexDirection: 'row', gap: spacing.sm, justifyContent: 'center', minHeight: 50, paddingHorizontal: spacing.lg },
