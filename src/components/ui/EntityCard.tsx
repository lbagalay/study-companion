...
export function EntityCard({ accent, badge, metadata, onPress, subtitle, title }: Props) {
  ...
  useEffect(() => {
    ...
    void AccessibilityInfo.isReduceMotionEnabled().then((reduceMotion) => {
      if (!mounted || reduceMotion) { entrance.setValue(1); return; }
      animation = Animated.timing(entrance, { duration: 360, easing: Easing.out(Easing.cubic), toValue: 1, useNativeDriver: Platform.OS !== 'web' });
      animation.start();
    });
    return () => { mounted = false; animation?.stop(); };
  }, [entrance]);
  return <Animated.View style={{ opacity: entrance, transform: [{ translateY: entrance.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) }] }}><Pressable accessibilityRole={onPress ? 'button' : undefined} onPress={onPress} style={({ pressed }) => [styles.card, { backgroundColor: palette.surface, borderColor: palette.border, opacity: pressed ? 0.78 : 1, transform: [{ scale: pressed ? 0.985 : 1 }] }]}>
    ...
    <View style={styles.trailing}>{badge ? <Text style={[styles.badge, { backgroundColor: palette.accentSoft, color: palette.accentStrong }]}>{badge}</Text> : null}<Ionicons color={palette.textMuted} name="chevron-forward" size={18} /></View>
...
const styles = StyleSheet.create({
  ...
  cornerWash: { borderRadius: radii.pill, height: 96, opacity: 0.08, position: 'absolute', right: -38, top: -34, width: 96 },
