...
export function MultiChoiceField<T extends string | number>({
  choices,
  error,
  label,
  onChange,
        ...
        {choices.map((choice) => {
          ...
          return (
            <Pressable
              ...
              onPress={() =>
                onChange(
                  selected ? value.filter((v) => v !== choice.value) : [...value, choice.value],
                )
              }
              style={({ pressed }) => [
                styles.choice,
            ...
            >
              <Text style={[styles.text, { color: palette.text }]}>{choice.label}</Text>
            </Pressable>
          );
        })}
      </View>
