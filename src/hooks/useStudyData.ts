...
export const keys = { subjects: ['subjects'] as const, schedules: ['schedules'] as const, assignments: ['assignments'] as const, exams: ['exams'] as const, materials: ['materials'] as const, notes: ['notes'] as const, pdfNotes: (materialId: string) => ['pdf-notes', materialId] as const, sessions: ['sessions'] as const };
...
export function usePdfNotes(materialId: string) { return useQuery({ queryKey: keys.pdfNotes(materialId), queryFn: () => service.listPdfNotes(materialId), enabled: Boolean(materialId) }); }
