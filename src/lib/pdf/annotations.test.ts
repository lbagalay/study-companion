...
describe('PDF ink annotation geometry', () => {
  it('normalizes pointer coordinates and clamps them to the page', () => {
    expect(normalizedInkPoint(60, 70, 0.8, { height: 200, left: 10, top: 20, width: 100 })).toEqual(
      { x: 0.5, y: 0.25, pressure: 0.8 },
    );
    expect(normalizedInkPoint(-20, 400, 0, { height: 200, left: 10, top: 20, width: 100 })).toEqual(
  ...
  it('ignores malformed stored strokes and clamps valid coordinates', () => {
    expect(
      parsePdfInkStrokes([null, { ...stroke, points: [{ x: 2, y: -1, pressure: 4 }] }]),
    ).toEqual([{ ...stroke, points: [{ x: 1, y: 0, pressure: 1 }] }]);
  });
});
