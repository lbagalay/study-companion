...
const pdfNoteDraftSchema = z.object({
  content: z
    ...
    .max(50000, 'Keep the note under 50,000 characters.'),
  pageNumber: z
    .number()
    .int()
    .min(1, 'Choose a valid PDF page.')
    .max(50000, 'Choose a valid PDF page.'),
  title: z
    .string()
    .trim()
...
export function parsePdfNoteDraft(
  input: { content: string; pageNumber: number; title: string },
  pageCount: number,
) {
...
export function pdfNoteValidationMessage(error: unknown) {
  if (error instanceof z.ZodError)
    return error.issues[0]?.message ?? 'Review the note and try again.';
