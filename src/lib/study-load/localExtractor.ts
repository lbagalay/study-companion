...
export async function extractStudyLoadLocally(
  ...
  if (Platform.OS !== 'web') throw new Error('Free study-load reading is currently available in the PWA.');
  if (asset.mimeType === 'application/pdf' || asset.name.toLowerCase().endsWith('.pdf')) {
    throw new Error('For free importing, save the study-load page as a screenshot and upload the image.');
  }
